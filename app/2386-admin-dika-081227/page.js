...
function ServicesManager({ token }) {
  ...
  const [open, setOpen] = useState(false)
  const [editing, setEditing] = useState(null)
  const [form, setForm] = useState({ name: '', description: '', price: 299000, icon: 'Package' })
  ...
  const save = async (e) => {
    ...
    try {
      ...
      setOpen(false); setEditing(null); setForm({ name: '', description: '', price: 299000, icon: 'Package' }); load()
    } catch (e) { toast.error(e.message) }
  }
  const del = async (id) => {
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between"><h2 className="text-2xl font-bold">Layanan Lainnya ({list.length})</h2><Button onClick={() => { setEditing(null); setForm({ name: '', description: '', price: 299000, icon: 'Package' }); setOpen(!open) }}><Plus className="w-4 h-4 mr-1" />Tambah Layanan</Button></div>
      {open && <Card><CardContent><form onSubmit={save} className="space-y-3"><div><Label>Nama Layanan</Label><Input required value={form.name} onChange={e => setForm({...form, name: e.target.value})} /></div><div><Label>Deskripsi</Label><Input value={form.description} onChange={e => setForm({...form, description: e.target.value})} /></div><div><Label>Harga</Label><Input type="number" value={form.price} onChange={e => setForm({...form, price: parseInt(e.target.value)})} /></div><div><Label>Icon</Label><Select value={form.icon} onValueChange={v => setForm({...form, icon: v})}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{iconOptions.map(ic => <SelectItem key={ic} value={ic}>{ic}</SelectItem>)}</SelectContent></Select></div><Button type="submit" className="bg-primary">{editing ? 'Update' : 'Simpan'}</Button></form></CardContent></Card>}
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
        {list.map(s => (
          <Card key={s.id}><CardContent className="p-4"><div className="flex justify-between"><div><h3 className="font-semibold">{s.name}</h3><p className="text-xs text-muted-foreground">{s.description}</p><div className="text-primary font-bold mt-1">{formatRupiah(s.price)}</div></div><div className="flex gap-1"><Button size="icon" variant="ghost" onClick={() => { setEditing(s); setForm({ name: s.name, description: s.description, price: s.price, icon: s.icon }); setOpen(true) }}><Pencil className="w-4 h-4" /></Button><Button size="icon" variant="ghost" onClick={() => del(s.id)}><Trash2 className="w-4 h-4 text-destructive" /></Button></div></div></CardContent></Card>
        ))}
      </div>
    </div>
  )
